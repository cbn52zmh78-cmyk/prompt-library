...
Credentials: prompt_library/.env (four OAuth 1.0a tokens).
"""
...
QUEUE_FILE = LIBRARY / "x_queue.json"
DM_QUEUE_FILE = LIBRARY / "x_dm_queue.json"
...
def get_client():
    ...
    if missing:
        raise RuntimeError(
            ...
            + ", ".join(missing)
            + f"\nCopy {LIBRARY / '.env.example'} to {ENV_FILE} and fill in values."
        )
